#!/usr/bin/env node
/**
 * Download master.dev HLS by fetching segments inside a browser session.
 * Usage: node scripts/masterdev-download.js '<curl command>' [output-name]
 */
...
function extractVideoHash(curl) {
  const m = curl.match(/\/([A-Za-z0-9]+)\/index(?:_|\.)/);
  return m ? m[1] : VIDEO_HASH;
}
...
async function main() {
  ...
  if (!curl) {
    console.error("Usage: node scripts/masterdev-download.js '<curl>' [filename]");
    process.exit(1);
  }

  fs.mkdirSync(DOWNLOADS, { recursive: true });
  const cookies = parseCookiesFromCurl(curl);
  const videoHash = extractVideoHash(curl);
  const workDir = path.join(DOWNLOADS, `work-${Date.now()}`);
  ...
  console.log(`Downloading ${segmentPaths.length} segments from ${variantUrl}...`);
  ...
  for (const line of body.split("\n")) {
    ...
    process.stdout.write(`\rSegment ${idx + 1}/${segmentPaths.length}: ${name}   `);
